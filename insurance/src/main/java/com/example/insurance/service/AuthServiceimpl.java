...
import com.example.insurance.repositary.DocumentRepo;
...
public class AuthServiceimpl implements AuthService {
	...
	public DocumentModel addDocument(MultipartFile file) throws IOException {
		...
		DocumentModel documentmodel=new DocumentModel(UUID.randomUUID().toString(),fileName,file.getContentType(),file.getBytes());
		return documentrepo.save(documentmodel);
	}
	
	@Override
	public DocumentModel getDocumentById(String id) {
		Optional <DocumentModel> fileOptional=documentrepo.findById(id);
